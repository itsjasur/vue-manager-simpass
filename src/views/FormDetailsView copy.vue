<template>
  ...
  <PrintablePopup v-if="printablePopup.active" />
  ...
  <div v-if="isPlanAvailable" class="container">
    <div class="partition">
      <template v-if="Object.keys(usimForms).length > 0">
        ...
        <div class="group" style="max-width: 100px">
          <label>유형</label>
          <input placeholder="" :value="selectPlansPopup.getTypeString()" readonly />
        </div>

        <div class="group" style="max-width: 100px">
          <label>통신망</label>
          <input placeholder="ASD" :value="selectPlansPopup.getCarrierString()" readonly />
        </div>

        <div class="group" style="max-width: 150px">
          <label>통신사</label>
          <input placeholder="ASD" :value="selectPlansPopup.getMvnoName()" readonly />
        </div>
          ...
          <div class="group" :style="{ maxWidth: item.maxwidth }">
            ...
            <template v-if="item.type === 'input'">
              <input
                ...
                :placeholder="item.placeholder"
                v-bind="inputBindings(index)"
                :class="{ 'input-clickable': index === 'usim_plan_nm' }"
              />
            </template>
    ...
    <div class="bottom-part">
      ...
      <template v-if="!signAfterPrintChecked">
        ...
        <div v-if="selectPlansPopup.type === 'PO' && !selfRegisterChecked" class="sign-container">
          <SignImageRowContainer
            type="payment"
        ...
        <div v-if="shouldShowAgreeContent && partnerNeedsToSign" class="sign-container">
          <SignImageRowContainer
            type="partner"
            @updated="updatePads"
            title="판매자 서명"
...
//when print popup is closed, page will be redirected to home
watch(
  () => printablePopup.active,
  (newValue, oldValue) => {
    if (oldValue === true && newValue === false) router.push('/')
  }
)
...
const submit = async () => {
  ...
  if (!signAfterPrintChecked.value) {
    ...
    //payment signs
    if (!selfRegisterChecked.value && selectPlansPopup.type === 'PO') {
